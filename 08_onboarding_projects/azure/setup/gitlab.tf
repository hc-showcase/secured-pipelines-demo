...
resource "gitlab_project_variable" "vault_azure_secret_backend_path" {
  project      = gitlab_project.secured-pipeline-project.id
  key          = "vault_azure_secret_backend_path"
  value        = file("../../../04b_configure_azure_secrets/temp_data/vault_azure_secret_backend_path")
  protected    = false
}

resource "gitlab_project_variable" "vault_azure_secret_backend_role" {
  project      = gitlab_project.secured-pipeline-project.id
  key          = "vault_azure_secret_backend_role"
  value        = vault_azure_secret_backend_role.azure-role.role
  protected    = false
}

resource "gitlab_project_variable" "arm_subscription_id" {
  project      = gitlab_project.secured-pipeline-project.id
...
resource "gitlab_project_variable" "arm_tenant_id" {
  project      = gitlab_project.secured-pipeline-project.id
  key          = "arm_tenant_id"
